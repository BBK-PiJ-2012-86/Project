...
public class Verifier {
	...
	public BitSet[][] request(Eqns eqns) {
		...
		// for rel testing
		for (int i = 0; i<3; i++) {
			result[0][300+2*i] = Rand.make(assSize);
			result[0][300+2*i+1] = Rand.make(assSize);
			result[1][300+i] = Manip.cross(result[0][300+2*i],result[0][300+2*i+1], assSize);
		}
		// ass testing
		for (int i = 0; i<2; i++) {
	...
	private void linear(BitSet[] amend, int size) {
		for (int i = 0; i<300; i+=3) {
			amend[i] = Rand.make(size);
			amend[i+1] = Rand.make(size);
